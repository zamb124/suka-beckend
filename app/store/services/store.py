...
from app.store.models import Stores
from app.store.schemas.store import LoginResponseSchema
...
class StoreService:
    ...
    async def get_store_list(self, limit: int = 12, token: str = None,) -> List[Stores]:
        query = select(Stores)
        if token:
